...
**Example**: If upgrading from Go 1.26 to Go 1.27:
```bash
# Create branch for previous version (1.26)
git checkout main
git checkout -b 1.26
...
- **File**: `.go-version`
  - Update to the new Go version (e.g., `1.27.0`)
...
```bash
# Use Microsoft Golang release versioning normally 1.27.0-1
...
- **File**: `.github/workflows/bump-golang.yml`
  - Update `go-minor` value to new minor version (e.g., `1.27`)

- **File**: `.github/workflows/bump-golang-previous.yml`
  - Update `go-minor` to previous minor version (e.g., `1.26`)
  - Update `branch` value to previous minor version (e.g., `1.26`)

#### 6. Configure Backport Support
- **File**: `.mergify.yml`
  - Add entry for new backport label: `backport-v1.26` (using previous version)

- **GitHub Labels**
  - Create new label at: https://github.com/elastic/golang-crossbuild/labels
  - Label name: `backport-v1.26` (using previous version)

#### 7. Commit and Create Pull Request in the `main` branch
```bash
git checkout main
git add -u
git commit -m "Update to Go 1.27.0"  # Use actual version
```

Create a Pull Request with:
- **Title**: `Update to Go 1.27.0`
...
2. Update main branch to Go 1.26
...
### Prerequisites
- Existing release tag (e.g., `v1.26.1`)
...
```bash
# Example for version 1.26.1
git checkout v1.26.1
git checkout -b 1.26.1.x  # 'x' is literal, not a placeholder
git push upstream 1.26.1.x
```

#### 2. Apply Your Changes

1. Ensure your original PR is merged to main
2. Comment on the PR: `@mergifyio backport 1.26.1.x`
...
```
Scenario: Critical security fix needed for Go 1.26.1

Steps:
1. git checkout v1.26.1
2. git checkout -b 1.26.1.x
3. git push upstream 1.26.1.x
...
- **Hotfix branches**: `1.26.1.x` (major.minor.patch.x format)
